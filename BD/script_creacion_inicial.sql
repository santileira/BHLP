...
INSERT INTO [ABSTRACCIONX4].[TIPOS_CUOTAS] (TIPOTARJ_COD,CUO_NUM) VALUES(2,1)
...
--American Express
INSERT INTO [ABSTRACCIONX4].[TIPOS_CUOTAS] (TIPOTARJ_COD,CUO_NUM) VALUES(3,1)
...
AS
	return (select distinct v.VIAJE_COD, v.AERO_MATRI,v.VIAJE_FECHA_SALIDA Fecha_Salida, v.VIAJE_FECHA_LLEGADAE Fecha_Llegada, 
			...
			from ABSTRACCIONX4.VIAJES v, ABSTRACCIONX4.RUTAS_AEREAS r1,
				ABSTRACCIONX4.CIUDADES c1, ABSTRACCIONX4.CIUDADES c2,ABSTRACCIONX4.SERVICIOS s, ABSTRACCIONX4.AERONAVES a
			where v.RUTA_ID = r1.RUTA_ID and
				r1.CIU_COD_O = c1.CIU_COD and
				r1.CIU_COD_D = c2.CIU_COD and
...
BEGIN
	...
	ELSE
		...
		IF(@FechaReinicio IS NULL)
			SET @Motivo = 'Se dio de baja por vida útil a la aeronave ' + CONVERT(varchar(10) , @Matricula) + ' del viaje'
		ELSE
...
AS
	BEGIN TRY
		...
		INSERT INTO ABSTRACCIONX4.SERVICIOS_RUTAS
		(RUTA_ID,SERV_COD)
		SELECT SCOPE_IDENTITY(),ABSTRACCIONX4.ObtenerCodigoServicio(S.Servicio)
		FROM (
		SELECT elemento 'Servicio' FROM @Servicios) S
...
AS
	...
	IF((SELECT COUNT(*) FROM ABSTRACCIONX4.PASAJES P , ABSTRACCIONX4.VIAJES V WHERE P.VIAJE_COD = V.VIAJE_COD AND V.RUTA_ID = @IdRuta AND (ABSTRACCIONX4.datetime_is_between(VIAJE_FECHA_SALIDA,[ABSTRACCIONX4].obtenerFechaDeHoy(),[ABSTRACCIONX4].FechaReinicioOMaxima(NULL)) = 1 OR ABSTRACCIONX4.datetime_is_between(VIAJE_FECHA_LLEGADA,[ABSTRACCIONX4].obtenerFechaDeHoy(),[ABSTRACCIONX4].FechaReinicioOMaxima(NULL)) = 1)) > 0 OR
		(SELECT COUNT(*) FROM ABSTRACCIONX4.ENCOMIENDAS E , ABSTRACCIONX4.VIAJES V WHERE E.VIAJE_COD = V.VIAJE_COD AND V.RUTA_ID = @IdRuta AND (ABSTRACCIONX4.datetime_is_between(VIAJE_FECHA_SALIDA,[ABSTRACCIONX4].obtenerFechaDeHoy(),[ABSTRACCIONX4].FechaReinicioOMaxima(NULL)) = 1 OR ABSTRACCIONX4.datetime_is_between(VIAJE_FECHA_LLEGADA,[ABSTRACCIONX4].obtenerFechaDeHoy(),[ABSTRACCIONX4].FechaReinicioOMaxima(NULL)) = 1)) > 0)
	BEGIN
	INSERT INTO ABSTRACCIONX4.DEVOLUCIONES (DEVOLUC_FECHA , DEVOLUC_MOTIVO)
	VALUES (ABSTRACCIONX4.obtenerFechaDeHoy() , @Motivo)
...
BEGIN
	BEGIN TRY
		...
		SELECT @IdRuta,
			   ABSTRACCIONX4.ObtenerCodigoServicio(S.Servicio)
...
AS
	...
	SET CANT_BUT_OCUPADAS = (SELECT COUNT(*) 
								FROM [ABSTRACCIONX4].PASAJES p
								WHERE p.PASAJE_CANCELADO = 0 AND
									p.VIAJE_COD = @viaje_cod)
	WHERE VIAJE_COD = @viaje_cod
...
AS 
	RETURN(
		...
		WHERE C.CLI_DNI = @dni AND C.CLI_APELLIDO = @ape AND VIAJE_FECHA_LLEGADA is NOT NULL AND VIAJE_FECHA_LLEGADA BETWEEN 
			[ABSTRACCIONX4].obtenerFechaDeHoy() - 365 AND [ABSTRACCIONX4].obtenerFechaDeHoy() AND ENCOMIENDA_CANCELADO = 0
...
BEGIN
	...
	OR [ABSTRACCIONX4].datetime_esMayor([ABSTRACCIONX4].obtenerFechaDeHoy() , V.VIAJE_FECHA_SALIDA) = 1)
