...
SELECT 1,/*(SELECT CLI_COD FROM [ABSTRACCIONX4].[CLIENTES] WHERE Cli_Dni = CLI_DNI AND Cli_Apellido = CLI_APELLIDO AND Cli_Nombre = CLI_NOMBRE 
		...
		(SELECT VIAJE_COD FROM [ABSTRACCIONX4].[VIAJES] WHERE 
			VIAJE_FECHA_LLEGADA = FechaLLegada AND 
			VIAJE_FECHA_LLEGADAE = Fecha_LLegada_Estimada AND 
			VIAJE_FECHA_SALIDA = FechaSalida AND
			AERO_MATRI = Aeronave_Matricula AND
			RUTA_ID = (SELECT RUTA_ID FROM [ABSTRACCIONX4].RUTAS_AEREAS R WHERE R.RUTA_COD = Ruta_Codigo AND R.RUTA_PRECIO_BASE_KG = MAX(Ruta_Precio_BaseKG) 
			AND R.RUTA_PRECIO_BASE_PASAJE = MAX(Ruta_Precio_BasePasaje) )
			),
