...
AS 
	RETURN(
		...
		WHERE C.CLI_DNI = @dni AND C.CLI_APELLIDO = @ape AND VIAJE_FECHA_LLEGADA is NOT NULL AND VIAJE_FECHA_LLEGADA BETWEEN 
			[ABSTRACCIONX4].obtenerFechaDeHoy() + 365  - 365 AND [ABSTRACCIONX4].obtenerFechaDeHoy() + 730 AND ENCOMIENDA_CANCELADO = 0 AND E.ENCOMIENDA_MILLAS !=0
...
AS 
	RETURN(
		SELECT	P.PASAJE_COD Codigo,
			...
			JOIN ABSTRACCIONX4.RUTAS_AEREAS R ON R.RUTA_ID = V.RUTA_ID
		WHERE C.CLI_DNI = @dni AND C.CLI_APELLIDO = @ape AND VIAJE_FECHA_LLEGADA is NOT NULL AND VIAJE_FECHA_LLEGADA BETWEEN 
			[ABSTRACCIONX4].obtenerFechaDeHoy() + 365  - 365 AND [ABSTRACCIONX4].obtenerFechaDeHoy() + 730 AND PASAJE_CANCELADO = 0 AND P.PASAJE_MILLAS != 0
